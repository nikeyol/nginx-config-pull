...
check_local_nginx_conf() {
  echo " * Testing nginx config on local"
  echo

  $NGINX -t
  if [ $? -ne 0 ]; then
    echo "nginx configuration check failed on local"
    return 1
    EXIT=2
  else
    return 0
...
restore_local_nginx_conf() {
  echo " * Restoring old configuration on local"
  echo
  ...
  for file in $CONFPATHS; do
    ...
    if $TEST -d $BACKUPDIR/$file; then
      $RSYNC -a $RSYNCVERB --delete $EXCLUDEARGS $BACKUPDIR/$file/ $file;
    else
      $RSYNC -a $RSYNCVERB --delete $EXCLUDEARGS $BACKUPDIR/$file $file;
    fi

    if [ $? -ne 0 ]; then
      echo "Failed to restore $file on local"
      echo "Please fix the issue manually."
      return 1
      EXIT=2
    fi
...
# modify the logic block
update_local_nginx_conf() {
  echo " * sync configuration from $1 to local"
  echo
  ...
  for file in $CONFPATHS; do
    ...
    if $TEST -d $file ; then
      eval $RSYNC -a $RSYNCVERB $RRSYNCPARAMS --delete $EXCLUDEARGS $1:$file/ $file
    ...
    if [ $? -ne 0 ]; then
      echo "Failed to update $file from $1 to local"
      echo "Please fix the issue manually."
      return 1
      EXIT=2
    fi
...
post_sync_local_replace() {
  if [ ! -z "$POSTSYNC" ]; then
    echo " * Adjusting configuration on local"
    echo
    ...
    for r in $POSTSYNC;do
      ...
      echo "Editing file $FILENAME on local"
      $SED -i'' $REPLACESTR $FILENAME
    done
  fi
...
main() {
  ...
  if [ -n "$CMPNODES" ]; then
    for n in $NODES; do
      n=$USERPREFIX$n
      echo "judge rsync remote connect, if success then execute"
...
# Add logic to pull remote to local
# Remove delete_remote_nginx_backup
# Add update_local_nginx_conf
  for n in $NODES; do
    echo "* test process in modify logic"
    n=$USERPREFIX$n
    check_remote_rsync $n || continue
    check_remote_nginx $n || continue

    if ! update_local_nginx_conf $n; then
      restore_local_nginx_conf
      continue
    fi

    if ! post_sync_local_replace; then
      restore_local_nginx_conf
      continue
    fi

    if ! check_local_nginx_conf; then
      restore_local_nginx_conf
      continue
    fi
    if $SERVICE nginx status > /dev/null ; then
      $SERVICE nginx reload && continue
      echo "nginx failed to reload on local, restoring previous config"
      restore_local_nginx_conf && $SERVICE nginx reload
    else
      echo "nginx is not running on local, please contact admin"
    fi
  done
